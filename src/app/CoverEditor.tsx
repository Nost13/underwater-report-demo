  ...
  return <section className="workspace cover-workspace">
    ...
    <div className="cover-editor-grid">
      <div className="panel cover-controls">
        ...
        <fieldset className="cover-performers"><legend>작업별 수행 주체</legend>{coverScopeGroups(sections).map(group=><label className="field" key={group.key}><span>{group.title}{group.supporting ? ' · 보조 작업' : ''}</span><select aria-label={`${group.title} 수행 주체`} value={scope.scopePerformers?.[group.key]??''} onChange={event=>{
