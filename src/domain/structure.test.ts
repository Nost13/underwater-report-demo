...
describe('report structure rules', () => {
  ...
  it('starts AFTER at editable CLEAN/R0 while BEFORE remains blank', () => {
    ...
    expect(conditions.BEFORE).toEqual({ fouling: { type: '', coverage: null, slimeOnly: false }, observed: { type: '', level: 'Normal / Trace' } });
    expect(conditions.AFTER).toEqual({ fouling: { type: 'Clean / No Fouling', coverage: 0, slimeOnly: false }, observed: { type: '', level: 'Normal / Trace' } });
  });
  ...
  it('expands replacement and appended services into unique phase-aware sections', () => {
    ...
    expect(sections.find((section) => section.service === 'POLISHING')?.conditions.AFTER)
      .toEqual({ fouling: { type: 'Clean / No Fouling', coverage: 0, slimeOnly: false }, observed: { type: '', level: 'Normal / Trace' } });
  });
